...
import { S2CellLayer, type S2CellLayerOptions } from './layer.js';
import { createLevelSelector } from './level.js';
import { cellsInView, type CellsInViewOptions, type CellsInViewResult } from './view.js';

/** Options for {@link S2ViewLayer}. */
export interface S2ViewLayerOptions extends S2CellLayerOptions, CellsInViewOptions {
  /**
  ...
  /** Called after every recompute, with the cells and level chosen. */
 ...
 *
 * Recomputes on camera movement, choosing a level from the view's ground
 * resolution through a {@link LevelSelector} so zooming steps between levels
 * instead of flickering between them, and coarsening further if the cell budget
 * demands it.
 */
export class S2ViewLayer {
  readonly layer: S2CellLayer;
  ...
  constructor(
    ...
    private readonly options: S2ViewLayerOptions = {},
  ) {
    this.layer = new S2CellLayer(scene, options);
    this.selector = options.selector ?? createLevelSelector(options);
